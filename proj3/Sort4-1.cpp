...
unsigned long long power10[13] = {
    1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 100000000, 10000000000, 100000000000, 100000000000000};

struct endS{
    Data *p;  
    unsigned long int k;
}; 
struct endS names[500][6000]; 
//firstName 
// for(auto j = l.begin(); j != l.back(); j++){ 
//   firstNamesOrdered[i, firstName] = array[j][2].append();
// }
bool lessthan(const Data *p1,const Data *p2) 
{ 
...
bool social(const Data *p1,const Data *p2 ){
  return (p1->ssn < p2->ssn); 
}
bool compare_long(unsigned long p1, unsigned long p2 ){
  return (p1 < p2); 
}

int person2int(Data* q){
    const char *ssn = q->ssn.c_str();
    long first = firstNamesOrdered[q->firstName];
    long last = lastNamesOrdered[q->lastName];
    long social = power10[8] * (ssn[0] - '0') + power10[7] * (ssn[1] - '0') +
        power10[6] * (ssn[2] - '0') + power10[5] * (ssn[4] - '0') +
        power10[4] * (ssn[5] - '0') + power10[3] * (ssn[7] - '0') +
        power10[2] * (ssn[8] - '0') + power10[1] * (ssn[9] - '0') +
        (ssn[10] - '0');
      //cout << last * power10[11] << ' ' << first * power10[12] << ' ' << social<< "\n"; 
    return first * power10[12] + social; 
}

bool compareFunct(struct endS q1, struct endS q2){ 
    return compare_long(q1.k, q2.k); 
}
bool social1(const Data *p1,const Data *p2 ){
  return (p1->ssn < p2->ssn); 
}
bool lessthan1(const Data *p1,const Data *p2) 
{ 
    int l = p1->lastName.compare(p2->lastName);
    if(l != 0){
      return (l<0);
    }
    l = p1->firstName.compare(p2->firstName);
    if(l != 0){
      return (l<0); 
    }
    return (p1->ssn < p2->ssn); 
...
if((l.front()->firstName == l.back()->firstName) && l.front()->lastName == l.back()->lastName) {
   // cout << "case 4 \n";
    int ssn_batch[1010000] = {0};
    string dash;
    int mini_batch_size = 0; 
    auto mini_batch_start = l.begin();
    auto mini_batch_end = l.begin();
    int added = 0;

    for( auto w = l.begin(); w != l.end(); w++){
        
        const char *ssn = (*w)->ssn.c_str();
        int social  = power10[8] * (ssn[0] - '0') + power10[7] * (ssn[1] - '0') +
        power10[6] * (ssn[2] - '0') + power10[5] * (ssn[4] - '0') +
        power10[4] * (ssn[5] - '0') + power10[3] * (ssn[7] - '0') +
        power10[2] * (ssn[8] - '0') + power10[1] * (ssn[9] - '0') +
        (ssn[10] - '0');

        ssn_batch[mini_batch_size] = social;
        mini_batch_size++; 
    }
    
      sort(begin(ssn_batch), begin(ssn_batch) + mini_batch_size);

    for(auto person = l.begin(); person != l.end(); person++){
      //cout << ssn_batch[added] << " : ";
      dash = to_string(ssn_batch[added]);
      auto new_str = string(9 - min(9, (int) dash.length()), '0') + dash;
      dash = new_str.substr(0,3) + '-' + new_str.substr(3,2)+ '-' + new_str.substr(5,4);
      //cout << dash << '\n';
      (*person)->ssn = dash; 
      ssn_batch[added] = 0;
      added++;
...
//standrd size array and copey back into l 
  }
//case 3 
else if(l.front()->firstName[0] == 'A' && l.back()->firstName[0] == 'Z'){
    //cout << "case 3 \n";
    int ssn_batch[2000] = {0};
    string dash;
    int mini_batch_size = 0; 
    auto mini_batch_start = l.begin();
    auto mini_batch_end = l.begin();
    ...
    for( auto w = l.begin(); w != l.end(); w++){
        if((*mini_batch_start)->firstName == ((*w)->firstName) && (*w)->lastName == ((*mini_batch_start)->lastName)){
        ...
        (ssn[10] - '0');
          ...
          ssn_batch[mini_batch_size] = social;
          mini_batch_size++; 
          ssn_batch[mini_batch_size+1] = 0;
        } else {
            
          sort(begin(ssn_batch), begin(ssn_batch) + mini_batch_size);
          added = 0;
          for(auto person = mini_batch_start; person != mini_batch_end; person++){
            //cout << ssn_batch[added] << " : ";
            dash = to_string(ssn_batch[added]);
            auto new_str = string(9 - min(9, (int) dash.length()), '0') + dash;
            dash = new_str.substr(0,3) + '-' + new_str.substr(3,2)+ '-' + new_str.substr(5,4);
...
//case 1 AND 2 
  ...
  for(auto it = l.begin(); it != l.end(); it++){
    int pos_j = lastNamesOrdered[(*it)->lastName];
    int pos_i = ++(names[pos_j][0].k);
    auto a = *it;
    ...
    int social = power10[8] * (ssn[0] - '0') + power10[7] * (ssn[1] - '0') +
        ...
        power10[2] * (ssn[8] - '0') + power10[1] * (ssn[9] - '0') +
        (ssn[10] - '0');
    names[pos_j][pos_i].k = (firstNamesOrdered[(*it)->firstName] * 10000000000 + social);
    names[pos_j][pos_i].p = a; 
  }
    l.clear();
for(int i = 0; i < 500; i++){
    sort(begin(names[i])+1, begin(names[i]) + names[i][0].k +1 , compareFunct);
        for(int j = 1; j <= names[i][0].k; j++){
            l.push_back(names[i][j].p);
            //names[i][j] / 1000000000 \\ decode back to the right struc;   
      }
    }
  }
}
