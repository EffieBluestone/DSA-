...
// int convertSocial2int(auto *it->ssn.c_str()){
//     return (power10[8] * (ssn[0] - '0') + power10[7] * (ssn[1] - '0') +
//       power10[6] * (ssn[2] - '0') + power10[5] * (ssn[4] - '0') +
//       power10[4] * (ssn[5] - '0') + power10[3] * (ssn[7] - '0') +
//       power10[2] * (ssn[8] - '0') + power10[1] * (ssn[9] - '0') +
//       (ssn[10] - '0') ) ;
// }
// int convertFn2int(auto *it){
//   return firstNamesOrdered[*it->firstName] ;
// }
// int convertLn2int(auto *it){
//   return lastNamesOrdered[*it->lastName] ; }
...
int person2int(Data* q){
    ...
    //intptr_t us this to cast it 
}
void sortDataList(list<Data*> &l) {
  // Fill this in
  for(auto it = l.begin(); it != l.end(); it++){
    int pos_j = lastNamesOrdered[(*it)->lastName];
    int pos_i = ++names[pos_j][0];
    names[pos_i][pos_j] = person2int((*it));
    //cout << names[pos_i][pos_j] << '\n';
  }
  int max = 0;
  for(int i = 0; i < 499; i++){
   // cout << names[i][0] << '\n';
...
// //case 3 
// if(l.front()->firstName[0] == l.front()->lastName[0]){
//     l.sort(social); 
//   }
